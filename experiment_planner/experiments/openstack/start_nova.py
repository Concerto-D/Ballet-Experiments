...
from ballet.assembly.simplified.type.openstack import nova_type
...
def setup(todo, myid, state_active, state_final):
    i = myid
    nova = DecentralizedComponentInstance(f"nova_{i}", nova_type())
    nova.connect_use_port(nova_type().get_port("mariadb_service"), f"mariadb_worker_{i}", "service")
    nova.connect_use_port(nova_type().get_port("keystone_service"), f"keystone_{i}", "service")
...
def run(todo, nworker, id, addresses, port, dir, latency=1, state_active="deployed", state_final="deployed", name=""):
    ...
    sending_time, planning_time, solving_time, inferred_constraints, generated_instructions, tlatency = \
        resolve.time_resolve(components, active, goal, goal_states, messaging, latency=latency, verbose=True)
    ...
    print(f"[DONE] node nova_{id}")
    for comp in plans:
        savePlan(plans[comp], f"{dir}/openstack_{nworker}_worker/nova_{id}")
    ...
    metrics = f"""nova_{id},{name},{nworker},{num_out_local_messages},{num_in_local_messages},{num_out_remote_messages},{num_in_remote_messages},{inferred_constraints},{generated_instructions},{setup_time},{get_message_time},{sending_time},{bhv_inference_time},{constraint_inference_time},{infer_message_time},{planning_time},{tlatency},{solving_time},{total_time}"""
